...
class EventModelTest(TestCase):
    ...
    def test_valid_status_choices(self):
        for status in ['draft', 'active', 'finished', 'cancelled']:
            e = Event.objects.create(name=f'E {status}', event_date=date(2026, 1, 1), status=status, created_by=self.user)
...
class EventInvitationTest(TestCase):
    ...
    def test_is_expired_when_past(self):
        inv = EventInvitation.objects.create(
            event=self.event, email='test@test.com',
            expires_at=timezone.now() - timedelta(hours=1), created_by=self.user,
        )
        self.assertTrue(inv.is_expired())

    def test_is_not_expired_when_future(self):
        inv = EventInvitation.objects.create(
            event=self.event, email='future@test.com',
            expires_at=timezone.now() + timedelta(days=7), created_by=self.user,
        )
...
class EventInstructorStrTest(TestCase):
    def setUp(self):
        ...
        self.instructor = Instructor.objects.create(full_name='Rosa Diaz', specialty='JS', email='rosa@test.com', created_by=self.user)
