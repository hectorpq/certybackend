...
class EventAdmin(admin.ModelAdmin):
    list_display = ('name', 'event_date', 'category', 'status_badge', 'instructor_count', 'enrollment_count', 'created_by')
    list_filter = ('status', 'event_date', 'category', 'created_at')
    search_fields = ('name', 'description', 'location')
    readonly_fields = ('id', 'created_at', 'updated_at', 'instructors_info', 'enrollments_info')
    ...
    fieldsets = (
        ...
        ('Schedule', {
            'fields': ('event_date', 'end_date', 'duration_hours')
        }),
        ('Location', {
            'fields': ('location',)
        }),
        ...
        ('Instructors', {
            'fields': ('instructors_info',),
            'classes': ('collapse',)
        }),
        ('Enrollments', {
            'fields': ('enrollments_info',),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
...
class EnrollmentAdmin(admin.ModelAdmin):
    list_display = ('student_name', 'event_name', 'enrolled_at', 'attendance_badge', 'grade')
    ...
    fieldsets = (
        ('Enrollment', {
            'fields': ('student', 'event', 'enrolled_at')
        }),
        ('Performance', {
            'fields': ('attendance', 'grade', 'notes')
        }),
    )
