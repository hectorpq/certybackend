...
class ExcelProcessingService:
    ...
    def process_records(self, records: List[Dict]) -> ExcelProcessingResult:
        ...
        try:
            ...
            # Log final
            logger.info(
                "Procesamiento completado: %s/%s exitosos",
    ...
    def _process_row(self, row: pd.Series, row_number: int):
        ...
        with transaction.atomic():
            ...
            logger.info(
                "Fila %s: Certificado enviado a %s en %s",
                row_number,
                participant.email,
                event.name,
            )
    ...
    def _get_or_create_participant(
        self, full_name: str, email: str, document_id: str, phone: str = None
    ) -> Participant:
        """
    ...
    def _create_certificate(
        ...
        if not created and self.template:
            ...
            logger.info(
                "Plantilla actualizada en certificado existente: %s - %s",
                participant.email,
                event.name,
            )
        elif created:
            ...
            logger.info(
                "Certificado ya existe (sin plantilla nueva): %s - %s",
                participant.email,
                event.name,
            )
