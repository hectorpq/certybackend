...
class DeliveryLogAdmin(admin.ModelAdmin):
    list_display = ('student_name', 'certificate_event', 'icons_display', 'status_badge', 'sent_at', 'sent_by')
    ...
    readonly_fields = ('id', 'sent_at', 'updated_at', 'certificate_info', 'delivery_info')
    ...
    def mark_as_successful(self, request, queryset):
        ...
        if count > 0:
            self.message_user(
                request,
                f"✅ {count} deliveries marked as successful",
                messages.SUCCESS
            )
