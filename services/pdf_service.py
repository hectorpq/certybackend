    ...
    def generate_certificate_pdf(certificate, template=None):
        ...
        try:
            ...
            expires_at   = (
                ...
                if certificate.expires_at else 'N/A'
            )

            PDFService._draw_text(
                c, student_name,
                layout.get('student_name', {}),
                PDFService.BASE_WIDTH / 2, PDFService.BASE_HEIGHT - 3.2 * inch,
                28, color_primary=HexColor('#1e3a8a'),
            )
            PDFService._draw_text(
                c, event_name,
                layout.get('event_name', {}),
                1.5 * inch, PDFService.BASE_HEIGHT - 4.0 * inch,
                18, color_primary=HexColor('#1e3a8a'),
            )
            PDFService._draw_text(
                c, f"Realizado el: {event_date}",
                layout.get('event_date', {}),
                1.5 * inch, PDFService.BASE_HEIGHT - 4.5 * inch,
                14, color_primary=HexColor('#94a3b8'),
            )

            # Bottom-left: verification code + validity (repositioned to avoid QR overlap)
            PDFService._draw_text(
                c, f"Código: {certificate.verification_code}",
                layout.get('verification_code', {}),
                0.5 * inch, 0.85 * inch,
                9, color_primary=HexColor('#64748b'),
            )
            PDFService._draw_text(
                c, f"Válido hasta: {expires_at}",
                {},
                0.5 * inch, 0.6 * inch,
                9, color_primary=HexColor('#64748b'),
            )
            ...
            PDFService._draw_qr_code(
                c, certificate.verification_code,
                layout.get('qr_code', {}),
            )
        ...
        if template and template.background_image:
            try:
                c.drawImage(
                    ...
                    0, 0,
                    width=PDFService.BASE_WIDTH, height=PDFService.BASE_HEIGHT,
                )
                return
            except Exception as exc:
