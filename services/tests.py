...
def make_cert(user=None, with_instructor=False, doc_id='11111', p_email='luis@test.com'):
    ...
    participant = Participant.objects.create(
        document_id=doc_id, first_name='Luis', last_name='Gomez',
        email=p_email, phone='999000111', created_by=user,
    )
    instructor = None
    if with_instructor:
        instructor = Instructor.objects.create(
            full_name='Dr. Ana Torres', email='ana@inst.com',
            specialty='Ingeniería de Software', created_by=user,
        )
    event = Event.objects.create(
        name='Workshop', event_date=date(2026, 6, 1),
        created_by=user, instructor=instructor,
    )
    template = Template.objects.create(name='Base', created_by=user)
    cert = Certificate.objects.create(
        participant=participant, event=event, template=template, generated_by=user,
    )
    return cert, user
...
class PDFTextOverflowTest(TestCase):
    ...
    def test_long_name_does_not_crash_generate(self, mock_path, mock_canvas):
        ...
        admin = User.objects.create_user(
            email='overflow_admin@test.com', full_name='Admin',
            password='pass', role='admin', is_staff=True,
        )
        event = Event.objects.create(
            name='Event ' + 'X' * 190,
            event_date=date(2026, 8, 1), created_by=admin,
        )
        participant = Participant.objects.create(
            document_id='OVF001',
            first_name='A' * 99,
...
class InstructorSignatureTest(TestCase):
    ...
    def test_generate_pdf_without_instructor_succeeds(self, mock_mkdir, mock_canvas):
        ...
        cert_no_instr, _ = make_cert(
            ...
            doc_id='99999', p_email='noinstr_p@test.com',
        )
...
class InstructorSignatureFieldTest(TestCase):
    ...
    def test_signature_url_field_still_exists(self):
        inst = Instructor.objects.create(
            full_name='Prof. Vargas', email='vargas@test.com',
            signature_url='https://example.com/sig.png', created_by=self.user,
        )
        self.assertEqual(inst.signature_url, 'https://example.com/sig.png')

    def test_instructor_str_unchanged(self):
        inst = Instructor.objects.create(
            full_name='Dr. Perez', specialty='Data Science',
            email='perez@test.com', created_by=self.user,
        )
