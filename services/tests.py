...
class EmailServiceTest(TestCase):
    ...
    def test_send_certificate_success(self, MockEmail):
        mock_msg = MagicMock()
    ...
    @patch('services.email_service.EmailMessage')
    def test_send_certificate_smtp_returns_zero(self, MockEmail):
        mock_msg = MagicMock()
    ...
    @patch('services.email_service.EmailMessage')
    def test_send_certificate_exception_returns_failure(self, MockEmail):
        MockEmail.side_effect = Exception('SMTP error')
        result = EmailService.send_certificate(self.cert, 'test@test.com')
        self.assertFalse(result['success'])
        self.assertIn('SMTP error', result['message'])

    @patch('services.email_service.EmailMessage')
    def test_send_bulk_all_success(self, MockEmail):
        mock_msg = MagicMock()
        mock_msg.send.return_value = 1
        MockEmail.return_value = mock_msg
    ...
    @patch('services.email_service.EmailMessage')
    def test_send_bulk_with_recipient_map(self, MockEmail):
        mock_msg = MagicMock()
        mock_msg.send.return_value = 1
        MockEmail.return_value = mock_msg
    ...
    @patch('services.email_service.EmailMessage')
    def test_send_bulk_failure_logged(self, MockEmail):
        mock_msg = MagicMock()
        mock_msg.send.return_value = 0
        MockEmail.return_value = mock_msg
        result = EmailService.send_bulk_certificates([self.cert])
    ...
    @patch('services.email_service.EmailMessage')
    def test_send_certificate_attaches_pdf_when_present(self, MockEmail):
        mock_msg = MagicMock()
        mock_msg.send.return_value = 1
        MockEmail.return_value = mock_msg
...
class PDFServiceTest(TestCase):
    ...
    def test_generate_pdf_success_no_template(self, mock_mkdir, MockCanvas):
        mock_c = MagicMock()
    ...
    @patch('services.pdf_service.canvas.Canvas')
    @patch('pathlib.Path.mkdir')
    def test_generate_pdf_with_template_no_bg(self, mock_mkdir, MockCanvas):
        mock_c = MagicMock()
        MockCanvas.return_value = mock_c
    ...
    @patch('services.pdf_service.canvas.Canvas')
    @patch('pathlib.Path.mkdir')
    def test_generate_pdf_canvas_exception(self, mock_mkdir, MockCanvas):
        MockCanvas.side_effect = Exception('canvas error')
    ...
    @patch('services.pdf_service.canvas.Canvas')
    @patch('pathlib.Path.mkdir')
    def test_generate_bulk_pdfs(self, mock_mkdir, MockCanvas):
        mock_c = MagicMock()
        MockCanvas.return_value = mock_c
