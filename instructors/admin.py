...
class InstructorAdmin(admin.ModelAdmin):
    list_display = ('full_name', 'email', 'specialty', 'status_badge', 'event_count', 'created_at')
    ...
    fieldsets = (
        ...
        ('Professional Details', {
            'fields': ('specialty', 'bio', 'signature_url')
        }),
        ('Status', {
            'fields': ('is_active',)
        }),
    ...
    def events_info(self, obj):
        ...
        return "\n".join([f"• {e.event.name} ({e.event.event_date.strftime('%d/%m/%Y')}) - {e.role}" 
                         for e in events])
